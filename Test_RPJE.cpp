...
map<pair<int, int>, pair<int, double>  > rule2rel;    // used to compose paths in testing R2
...
public:
					...
					if (rel_path.size() > 1){
                           for (int i = 0; i < rel_path.size(); i++){
                                if (rule2rel.count(make_pair(rel_path[i], rel_path[i+1])) > 0){
                                    ...
                                    confi_path = confi_path * confi_integ; // 组合路径规则置信度的乘积
    ...
    void run()
    {
        FILE* f1 = fopen(("./data_FB15K/res/relation2vec_rule70_5.txt"+version).c_str(),"r");
        FILE* f3 = fopen(("./data_FB15K/res/entity2vec_rule70_5.txt"+version).c_str(),"r");
		...
		int hit_n = 10;				// Hits@n: default n=10，可以修改
...
void prepare()
{
	cout<<"------------The test process for RPJE_rule!------------\n";
	...
	fclose(f2);
    FILE* f_kb = fopen("./data_FB15K/test_pra.txt","r");
    ...
    fclose(f_kb);
    FILE* f_path = fopen("./data_FB15K/path2.txt","r");
    ...
    fclose(f_path);
    FILE* f_kb1 = fopen("./data_FB15K/train.txt","r");
    ...
    fclose(f_kb1);
    FILE* f_kb2 = fopen("./data_FB15K/valid.txt","r");
    ...
    int rel1, rel2, rel3;
    double confi;
    FILE* f_rulepath = fopen("./data_FB15K/rule/rule_path70.txt","r");
